...
import { postAPI, postAPIFunction, putAPIFunction,  putAPI, deleteAPIFunction, getAPI, postFormDataAPIFunction, getAPIFunctionBlob } from "../../apiCalls/functions/index";
import { ADD_STACK, UPDATE_STACK, DELETE_STACK, SEARCH_STACK } from "../../apiCalls/urls/executionModule/stack";

import { IMPORT_USERS, EXPORT_USERS } from "../../apiCalls/urls/executionModule/users";
...
const importUsers = async (file: any) => {
    ...
    const result: any = await postFormDataAPIFunction(IMPORT_USERS, formData);
    return result
}

const exportUsers = async () => {
    
    const result: any = await getAPIFunctionBlob(EXPORT_USERS);
    console.log(result);
    let fileName =  'users' + Date.now() + '.xlsx';
...
function* importFunction(action: any): any {
    ...
    try {
        yield put(apiLoading());
        let result = yield call(importUsers, file)
        console.log(result)
        if (result.status) {
            yield put(apiMessage("Success"))
        }
...
function* exportFunction(action: any): any {
    ...
    try {
        yield put(apiLoading());
        let result = yield call(exportUsers)
        if (result.status) {
            yield put(apiMessage("Success"))
        }
        else
...
export function* userSaga() {
    ...
    yield takeLatest(ACTION_TYPES.IMPORT_USERS, importFunction);
    yield takeLatest(ACTION_TYPES.EXPORT_USERS, exportFunction);
}
