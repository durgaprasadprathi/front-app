...
import { importWorkspace, exportWorkspace } from "./actions";
...
const WorkspaceSection = (props: any) => {
    ...
    const handleWorkspace = (id: any) => {
        history.push('/canvas?workspace_id=' + id)
    }
        ...
        allData && allData.map((a: any) => {
            ...
            _rows.push({
                ...
                action: <>
                    {/* <SimpleButton
                        title="Go Workspace"
                        onClick={() => handleWorkspace(a.workspaceId)}
                    />
                    &nbsp;&nbsp;&nbsp; */}
                    <i
    ...
    const editWorkspace = async (data: any) => {
        ...
        const allData: any = await putAPI(UPDATE_WORKSPACE + selectedItem.workspaceId, _data)
        console.log(allData);
        if (allData.status === "success") {
            setOverLay(false);
            props.apiMessage("Success")
    ...
    const deleteWorkspace = async () => {
        ...
        // console.log(myArray);
        const allData: any = await deleteAPIFunction(DELETE_WORKSPACE, { ids: myArray })
        if (allData.status) {
            props.apiMessage("Success")
        ...
        <>
            ...
            <div className="page-content">
                ...
                <Workspace
                    ...
                    section="WORKSPACE"
                    import={props.importWorkspace}
                    export={props.exportWorkspace}
                />
                {
                    overlay
                        ?
...
const mapDispatchToProps = { apiLoading, apiMessage, apiErrorMessage, importWorkspace, exportWorkspace };
